...
class GraphReward(GraphScene):
    ...
    def drawGraph(self):
        ...
        label = TextMobject("How much you care \\\\ about future rewards")
        label.scale(0.9)
        label.to_edge(LEFT)
        ...
        self.add(rects)
        self.rects = rects
        group = VGroup(*[rects,self.axes])
        group.scale(0.9)
        group.to_corner(RIGHT)
        group.add(label)
        self.play(ShowCreation(self.rects))
        self.play(ShowCreation(label))
        self.play(group.scale,0.5,group.to_corner,RIGHT+UP)
        self.play(group.scale,0.5,group.to_corner,LEFT+UP)
        ...
        # self.play(
                ...
                # )
        self.dither(2)
        self.play(group.fade,1.0)
        
        discounted_r = TexMobject("$$\sum_{i=0}^{h-1} \gamma^t r_{t+i}$$").scale(1.5)
        discounted_r.to_edge(LEFT)
        d2 = discounted_r.deepcopy()
        d2.next_to(discounted_r,DOWN,buff=MED_LARGE_BUFF)
        self.play(ShowCreation(d2))
        self.play(Write(discounted_r))
        self.dither(5)
